...
const App: React.FC = () => {
  ...
  return (
    <section className="main-container">
        ...
        <>
          <div className="cards-container-grid">
            <Card
              ...
              info={`${currencySymbol} ${stockData.info.book_value.toFixed(2)}`}
              additionalInfo={`Compared to ${currencySymbol}${penultimatePrice}  (previous day's closing price)`}
            />
            <Card
              title="Dividend per Share"
              info={`${currencySymbol} ${stockData.info.dividend_per_share.toFixed(
                2
              )}`}
            />
            <Card
              title="Dividend Yield"
              info={`${(stockData.info.dividend_yield * 100).toFixed(2)}%`}
            />
          </div>
          <div className="cards-container-grid">
            <Card
              title="Earnings per Share"
              info={stockData.info.eps.toFixed(2)}
            />
            <Card
              title="P/E Ratio"
              variation={"help-icon"}
              ...
              info={stockData.info.p_e_ratio.toFixed(2)}
            />
            <Card
              title="Profit Margin"
              info={stockData.info.profit_margin.toFixed(2)}
            />
          </div>
        </>
      )}
      {recentPrices && stockData && !loading && !error && (
        <>
          ...
          <div className="cards-container-grid">
            <Card
              title="Forward P/E"
              info={stockData?.analysis.forward_pe.toFixed(2)}
            />
            <Card
              title="Price to Sales"
              info={stockData?.analysis.price_to_sales.toFixed(2)}
            />
            <Card
              title="Price to Book"
          ...
          <div className="cards-container-grid">
            <Card
              ...
              info={(stockData?.analysis.return_on_assets * 100).toFixed(2)}
            />
            <Card
      ...
      {dividendByYear && stockData && !loading && !error && (
        <>
          ...
          <div className="cards-container-grid">
            <Card
              ...
              info={`${currencySymbol} ${stockData?.analysis.graham_formula_value.toFixed(
                2
              )}`}
            />
            <Card
              ...
              info={`${stockData?.analysis.peter_lynch_ratio.toFixed(2)}`}
            />
            <Card
              ...
              info={`${currencySymbol} ${stockData?.analysis.projected_ceiling_price.toFixed(
                2
              )}`}
            />
          </div>
