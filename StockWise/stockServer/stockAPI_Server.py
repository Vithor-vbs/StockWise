...
def get_stock_data():
    try:
        ...
        desired_dividend_yield = float(
            request.args.get('desired_dividend_yield'))
        ...
        try:
            ...
            graham_formula_value = None

        try:
            peter_lynch_ratio = p_e_ratio / (0.10 * 100) if p_e_ratio else None
        except TypeError:
            peter_lynch_ratio = None

        try:
            total_dividends_accumulated = sum(
                earnings) if len(earnings) > 0 else None
        except TypeError:
            total_dividends_accumulated = None
